    ...
    for lr in [0.005, 0.01, 0.05]:
        for regul in [0.001, 0.005, 0.01]:
            for hiddensize in [100, 150, 200, 250, 300]:
                ...
                a.train(5)
                print(str(lr) + '/' + str(regul) + '/' + str(hiddensize) + '模型在validation set准确率为' + str(a.predict(testdata, testlabel)))
...
def finalmodel(lr, regu, hsize):
    ...
    plt.legend()
    # plt.show()
    plt.savefig('figures/acc_loss.jpg')

if __name__ == '__main__':
    # 获取训练集/测试集 nparray型数据和标签
    traindata, valid_data = images_data('train')
    trainlabel, valid_label = labels_data('train')
    ...
    output = findbest(traindata, valid_data, valid_label)  # 网格搜索找到较优的超参 
    ...
    plt.plot(x, y)
    # plt.show()
    plt.savefig('figures/acc_hyper.jpg')
    ...
    print('在测试集上的准确率：', predictbymodel(testdata, testlabel))
